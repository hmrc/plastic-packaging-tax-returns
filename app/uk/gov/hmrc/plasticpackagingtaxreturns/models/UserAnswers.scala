...
case class UserAnswers(id: String, data: JsObject = Json.obj(), lastUpdated: Instant = Instant.now) {
  ...
  /** Fills the given form by reading a user answer and passing that through the given function
    ...
    */
  def fillWithFunc[AnswerType, FormValue](gettable: Gettable[AnswerType], form: Form[FormValue], func: AnswerType => Option[FormValue])(implicit
    reads: Reads[AnswerType]
  ): Form[FormValue] =
    get(gettable)
      .flatMap(func)
      .map(form.fill)
  ...
  /**
    * Overload of [[UserAnswers#getOrFail(queries.Gettable, play.api.libs.json.Reads, scala.reflect.api.TypeTags.TypeTag)]]
   ...
   * @param question           - the user-answer we might be changing
    ...
    */
  def change[A](question: QuestionPage[A], newValue: A, saveUserAnswerFunc: SaveUserAnswerFunc)(implicit format: Format[A]): Future[Boolean] = {
  ...
  /** Change the value or the given user answer, or add the user answer if it does not exists. The value of the user
    ...
    */
  def changeWithFunc[A](question: QuestionPage[A], newValueFunc: Option[A] => A, saveUserAnswerFunc: SaveUserAnswerFunc)(implicit
    format: Format[A],
    ec: ExecutionContext
  ): Future[Unit] = {
