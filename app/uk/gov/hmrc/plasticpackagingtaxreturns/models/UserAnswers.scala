...
case class UserAnswers(id: String, data: JsObject = Json.obj(), lastUpdated: Instant = Instant.now) {
  ...
  /** Overload of [[UserAnswers#fill(queries.Gettable, play.api.data.Form, play.api.libs.json.Reads)]]
    * @param gettable
    *   source of path of user answer
    * @return
    *   filled form
    */
  def fill[A](gettable: Gettable[A], form: Form[A])(implicit rds: Reads[A]): Form[A] = fill(gettable.path, form)

  /** Reads a user answer to fill given form
    * @param path
    *   path to user answer to read
    * @param form
    *   form to fill
    * @tparam A
    *   expected type of user answer to read
    * @return
    *   a filled form
    */
  def fill[A](path: JsPath, form: Form[A])(implicit rds: Reads[A]): Form[A] = get(path).fold(form)(form.fill)

  /** Fills the given form by reading a user answer and passing that through the given function
    * @param gettable
    *   [[Gettable]] provides [[JsPath]] of the user answer
    * @param form
    *   [[Form]] to fill with answer
    * @param func
    *   function to extract form value from user answer
    * @param reads
    *   [[Reads]] to de-serialise user answer
    * @tparam AnswerType
    *   type of the user answer
    * @tparam FormValue
    *   type of the form's value
    * @return
    *   the form as is, or a new form filled with the value returned by the given function. If the function return
    */
  def fillWithFunc[AnswerType, FormValue](
    gettable: Gettable[AnswerType],
    form: Form[FormValue],
    func: AnswerType => Option[FormValue]
  )(implicit reads: Reads[AnswerType]): Form[FormValue] =
    get(gettable)
      .flatMap(func)
      .map(form.fill)
      .getOrElse(form)
  ...
  /** Read a user answer or return None
    * @param question
    *   source of the path to the answer to read
    * @tparam A
    *   the expected type of the answer
    * @return
    *   Some[A] with user answer or None
    */
  def get[A](question: Gettable[A])(implicit rds: Reads[A]): Option[A] = get(question.path)

  /** Read a user answer or return None
    * @param path
    *   the path to the answer to read
    * @tparam A
    *   the expected type of the answer
    * @return
    *   Some[A] with user answer or None
  ...
  /** @param question
    *   source of path to read from user answers
    * @tparam A
    *   the expected type of the answer
    * @return
    *   the answer if found as an object of type A
  ...
  /** Tries to set the given answer to the given path. Over-writes an existing answer at the path, or adds a new answer
    * @param question
    *   source of path to write answer to
    * @param value
    *   answer to write
    * @param cleanup
    *   true to call question's clean-up method (be careful, can be a circular call)
    * @tparam A
    *   inferred type of value
    * @return
    *   an updated [[UserAnswers]]
    */
  ...
  /** Sets the answer (value) to the given question (settable), or fails if something goes wrong - unlike set() which
    * returns a failed Try
    * @param settable
    *   \- question we're setting the answer / data for
    * @param value
    *   \- value to set
    * @param cleanup
    *   \- true to call the question's cleanup method
    * @tparam A
    *   \- type of 'value'
    * @return
    *   Future of updated UserAnswers
    */
  ...
  /** Saves this UserAnswers using the given function
    * @param saveUserAnswerFunc
    *   \- function called to save user answers
    * @return
    *   Future of updated UserAnswers
    */
  ...
  /** If user's answer has changed, passes updated user-answers object to given save function
    *
    * @param question
    *   \- the user-answer we might be changing
    * @param newValue
    *   \- the user's answer
    * @param saveUserAnswerFunc
    *   \- function to call if answer has changed
    * @param format
    *   \- formatter for user's answer object type
    * @tparam A
    *   \- type of user's answer
    * @return
    *   - Future of false if user's answer is the same as the current value
    *   - Future of true if user's answer has changed
    */
  def change[A](question: QuestionPage[A], newValue: A, saveUserAnswerFunc: SaveUserAnswerFunc)(implicit
    format: Format[A]
  ): Future[Boolean] = {
  ...
  /** Change the value or the given user answer, or add the user answer if it does not exists. The value of the user
    * answer will be the value returned by `newValueFunc(previousValue)`, where `previousValue = None` if the user
    * answer does not exist
    * @param question
    *   source of path
    * @param newValueFunc
    *   function to calculate new value, passed Some(previous value) if there was one, or None if there was not a
    *   previous value
    * @param saveUserAnswerFunc
    *   function to save the updated user answers once new value applied
    * @tparam A
    *   type of user answer field's value
    * @return
    *   [[Future]] of Unit
    */
  def changeWithFunc[A](
    question: QuestionPage[A],
    newValueFunc: Option[A] => A,
    saveUserAnswerFunc: SaveUserAnswerFunc
  )(implicit format: Format[A], ec: ExecutionContext): Future[Unit] = {
  ...
  /** Alternative to removePath
    * @param question
    *   source of path to remove
    * @param cleanup
    *   true to call question's clean-up function (be-careful, can be a circular call)
    * @return
    *   [[Try]] of updated [[UserAnswers]]
    */
  ...
  /** Quickly set lots of (top-level) fields. Can be either
    * @param all
    *   varargs of (key: String, value: A) or (key: String, value: JsValue)
    * @param writes
    *   [[Writes]] for A
    * @tparam A
    *   allows values without eg JsNumber wrapper, if all values are same type
    * @return
    *   [[UserAnswers]] with previous answer merged with given key-value
    * @see
    *   UserAnswerSpec
    */
